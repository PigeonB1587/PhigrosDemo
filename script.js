			...
			function loading(num) {
				...
				if (num == zipData.length) {
					if (selectchart.children.length == 0) {
						message.sendError("读取出错：未发现谱面文件"); //test //test
...
function qwqdraw1(now) {
	...
	for (const i of clickEvents1) { //绘制打击特效1
		const tick = (now - i.time) / i.duration;
		ctxos.globalAlpha = 1;
		ctxos.setTransform(...imgFlip(noteScale * 4, 0, 0, noteScale * 4, i.offsetX, i.offsetY)); //缩放
		ctxos.drawImage(i.images[parseInt(tick * 30)] || i.images[i.images.length - 1], -306, -292); //停留约0.5秒
		ctxos.fillStyle = i.color;
		ctxos.globalAlpha = 1 - tick; //不透明度
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.textAlign = "center";
	ctxos.font = `${lineScale * 0.8}px NoirPro`;//分数大小
	ctxos.textAlign = "right";
	ctxos.textAlign = "center";
	ctxos.globalAlpha=0.8;
	ctxos.fillText(stat.scoreStr, canvasos.width - lineScale * 16.6, lineScale * 0.80);//分数位置
	...
	if (stat.combo > -1) {
		...
		ctxos.globalAlpha=0.8;
		ctxos.fillText(stat.combo, wlen, lineScale * 1.8);//combo位置
...
function qwqdraw3(statData) {
	...
	ctxos.drawImage(res["LevelOver1"], 0, 0);//结算背景
	ctxos.globalCompositeOperation = "source-over";
	ctxos.globalAlpha = 1;
	//歌名和等级
	ctxos.fillStyle = "#fff";
	...
	ctxos.drawImage(res["Ranks"][stat.rankStatus], 960 - qwq3 / 2, 745 - qwq3 / 2, qwq3, qwq3);//rank位置
	//各种数据
	...
	ctxos.fillText(stat.scoreStr, 955, 444);//分数位置
